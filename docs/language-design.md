...
- JavaScript 的灵活：动态值、一等函数、对�?数组友好、脚本式开发体�?

推荐执行模型�?

```text
源码 -> Lexer -> Parser -> AST -> 语义分析 -> 字节�?-> VM 执行
```
...
结合当前代码�?`docs/architecture-analysis-report.md`，Icoo 已经不是只覆盖最小表达式求值的原型，而是一套包含语言前端、字节码 VM、模块系统、标准库�?CLI 工具链的完整实现�?
...
下面这部分描述的�?*当前实现中已经可用或已经落到代码中的能力**，不是远期设想�?
...
- 如果表达式结果不�?`error`，直接返回该�?
- 如果结果�?`error`，则向外提前传播该错误�?
- 如果外层�?`finally`，会先执�?`finally`
...
- `satisfies(value, InterfaceName)` 运行时检�?

当前类型系统更偏**声明与约束表�?*，而不是完整静态类型推导系统�?
...
- `<` 已经承担比较运算符语�?
- 若继续复�?`<` 表示继承，会让类声明与表达式语法共享同一符号入口
- 改用 `<-` 后，`<` 只表示比较，`<-` 只表示继承，词法和语法边界更清晰

这是一项明确的可读性优先决策，而不是为了追求和其他语言表面形式一致�?
...
运行时实现上，`go` 任务不是共享同一个活�?VM，而是由主 VM 调度、在�?VM 中隔离执行闭包或原生函数。这能减少解释器内部共享状态带来的并发复杂度�?
...
- `check` 走词法、语法、语义分析，不进入编译执�?
- `run` 走完整主链：词法 -> 语法 -> 语义 -> 编译 -> VM 执行
- `repl` 会对纯表达式自动包装 `return ...`，以便直接回显结�?
- `bundle` 打的�?*源码级归�?*，不是字节码级归�?
- `build` 会把 bundle 追加�?CLI stub，生成可分发执行文件

---

## 并发风格

采用对象风格 channel API，而不�?Go �?`<-` 操作符�?
...
ImportDecl      = "import" ImportPath [ "as" Identifier ]
                | "from" ImportPath "import" ImportSpec { "," ImportSpec } ;
...
更多使用示例�?`docs/iterators.md`�?

Icoo 当前�?`for-in` 基于统一迭代器协议，而不是为数组单独做语法特判�?
...
对象还支持覆盖默认迭代行为。如果对象自身存�?`iter` 字段，优先使用该字段�?
...
上面�?EBNF 仍然是“靠近实现的草案”，不是形式化规范。当前仓库里有几处实现细节值得额外说明�?
...
- 类方法目前不�?`fn` 关键字，直接写方法名
- 装饰器既可修饰函数，也可修饰类；类内部的方法也可单独带装饰器
- `interface` 方法参数在当前实现里记录的是“参数名 + 类型表达式”序列，文档中的 EBNF 只表达结构，不试图精确反映内�?AST 细节
...
- `chan()` 编译�?builtin function 调用
- `ch.send(v)` / `ch.recv()` 编译成方法调�?
...
`select` 建议首版编译�?runtime intrinsic 调用，而不是一开始就设计复杂 opcode�?

即：
- 编译器先构�?select case 描述
- 调用内建 `__select(cases)`
...
MVP 不需要专�?pattern opcode，先�?`match` 编译成条件链�?
复杂数组/对象模式匹配可在后续考虑�?
...
Go error 主要用于 VM/编译�?宿主失败；脚本运行时异常建议使用 `ErrorValue`�?
...
整个模块建议编译成一个隐式初始化函数，例�?`__module_init__`�?
...
- 当前函数找不�?local
- 递归 parent 查找
- 若命�?parent local，则标记 `IsCaptured=true`
- 当前函数登记对应 upvalue
...
其中 `type/interface` �?MVP 可先只做语义记录�?
...
- 数组：逐个压栈�?`OpArray count`
- 对象：逐个�?key/value �?`OpObject pairCount`
...
建议�?
- `match` MVP 降级成条件跳转链
- `try/catch` 第二阶段实现，依赖异常处理器�?
- `go` 第一版可降级�?builtin `__go`
- `ch.send/ch.recv` 第一版仍作为普通成员调�?
- `select` 第二阶段实现�?runtime intrinsic
...
2. **Stdlib Modules**：通过 `import` 使用，如 `std.io`、`std.time.basic`、`std.data.json`
...
#### `panic(message)`
MVP 先作�?runtime error 返回；后续再接入 `try/catch` 的可捕获异常体系�?
...
首版推荐使用 **Go 原生构造模�?*，而不是先�?`.ic` 源码实现标准库�?
...
func (vm *VM) registerStdlib() {
	...
	vm.stdlib["std.time.basic"] = loadStdTimeModule
	vm.stdlib["std.data.json"] = loadStdJSONModule
	vm.stdlib["std.io.fs"] = loadStdFSModule
	vm.stdlib["std.math.basic"] = loadStdMathModule
}
```
...
也就是说，当�?`pkg/api` 更像一�?*运行、检查、bundle 执行门面**，而不是一个完整的宿主嵌入 SDK。像 `DefineFunc`、`DefineValue`、`DefineModule` 这类直接注入接口，目前并没有作为 `pkg/api` 的公开方法提供�?
...
对宿主开发者来说，最重要的边界是：`pkg/api` 返回的是 `runtime.Value`，而不�?Go 原生类型。因此在宿主集成时，通常需要：
...
而不适合直接把任�?Go struct 通过反射无约束暴露到脚本层�?
...
2. 为常�?`runtime.Value` 提供安全�?helper 转换函数
3. 在不引入反射泛滥的前提下补最小注入接�?
4. 最后再考虑更完整的宿主对象桥接
