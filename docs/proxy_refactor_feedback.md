...
即便是 `examples/proxy/lib/store.ic` 这样很小的请求记录存储，也会很快冒出一层重复的数据访问样板：
...
这说明在 `std.db` 之上，还缺一层足够轻但足够顺手的查询/模型抽象。它不需要去复制 GORM，但应该能覆盖脚本服务里最常见的 CRUD 和链式查询。
...
- `create()`
- `insert()`
- `where(...).first()/get()/all()/count()`
- `update()` / `delete()`
- `select()` / `orderBy()` / `limit()` / `offset()`

它解决的是 `store.ic` 这类“明明业务很小，却不得不写很多 SQL 样板”的问题。
