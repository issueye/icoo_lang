...
3. 补齐字符串键对象字面量，以及服务端请求对象的 `req.header(name)` / `req.hasHeader(name)`。
4. 用这些能力重写了 `examples/proxy` 中最典型的对象访问、header 读取与模块缓存样板。
5. 增加对应运行时测试，确保模块可用。
...
2. 以标准库形式补一个“代理友好”的 HTTP 能力层，而不是直接追求更多语法。
